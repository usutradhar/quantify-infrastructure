...
data <- read.csv('D:/Work/Box Sync/Quantify Infrastructure/output_csvs/df_grouped_ssp2_1.csv')
data$city <- NA
...
data <- read.csv('D:/Work/Box Sync/Quantify Infrastructure/output_csvs/df_grouped2_ssp2_1.csv')
...
data <- read.csv('D:/Work/Box Sync/Quantify Infrastructure/output_csvs/df_grouped3_ssp2_1.csv')
# data <- dplyr::filter(data, CensusPop_20 <= 10000) 
sum(is.na(data))
data$city <- NA
...
p
outputfile =  'D:/Work/Box Sync/Quantify Infrastructure/Figures/below_10000'
...
data <- read.csv('D:/Work/Box Sync/Quantify Infrastructure/output_csvs/df_grouped_ssp2_2.csv')
data$city <- NA
data$city <- factor(data$city.type, levels = c("urban", "suburban", "periurban", "rural"))
data <- data[order(data$city), ]
p1 <- plot_median_density(DF = data, X = "volume_m3_perCap_2020", Y = "length_m_perCap_2020", GROUP="city",
...
data <- read.csv('D:/Work/Box Sync/Quantify Infrastructure/output_csvs/df_grouped2_ssp2_2.csv')
data$city <- NA
data$city <- factor(data$citytype_at_2050, levels = c("urban", "suburban", "periurban", "rural"))
data <- data[order(data$city), ]
p2 <- plot_median_density(DF = data, X = "volume_m3_perCap_2050", Y = "length_m_perCap_2050", GROUP="city",
...
data <- read.csv('D:/Work/Box Sync/Quantify Infrastructure/output_csvs/df_grouped3_ssp2_2.csv')
data$city <- NA
data$city <- factor(data$citytype_at_2100, levels = c("urban", "suburban", "periurban", "rural"))
data <- data[order(data$city), ]
p3 <- plot_median_density(DF = data, X = "volume_m3_perCap_2100", Y = "length_m_perCap_2100", GROUP="city",
...
p <- plot_grid( prow, legend_b, ncol = 1, rel_heights = c(1, .2))
p
outputfile =  'D:/Work/Box Sync/Quantify Infrastructure/Figures/above_10000'
...
plot_median_density = function(DF=df, X="", Y="", GROUP="", title_text = ""){
  ...
  return(
    ggplot(DF,               
      ...
      # theme_tq() +
      # labs(title = paste0("Median values for states for cities with 10,000 population or less by " ,  title_text))+ 
      scale_x_continuous(name="Per capita residential built volume (m3)", limits=c(200, 1350)) +
      scale_y_continuous(name="Per capita roadway length (m)", limits=c(-5, 120))
  )
}

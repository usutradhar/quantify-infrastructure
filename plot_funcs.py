...
def plot_distribution(data, xlims = [-10, 25], ylims = [0,1], label_text = 'x_label', p95_position=4000):
    ...
    plt.savefig(r'D:\Work\Box Sync\Quantify Infrastructure\Figures\\' + label_text + '.png', dpi = 300, transparent=True,  bbox_inches='tight')
...
def plot_burden_bar(df, burdencol):
    ...
    ax.set(xlim=(0, 14000))
    ax.invert_xaxis()
    # plt.savefig(r'D:\Work\Box Sync\Quantify Infrastructure\Figures\burden_plot_' + col_name +'.png', transparent=True,  bbox_inches='tight')
    # 


# Plot sankey plots of burden extent
def plot_burden_sankey(df,trend_col,infra_col, output_path = r"D:\Work\Box Sync\Quantify Infrastructure\Figures\\"):

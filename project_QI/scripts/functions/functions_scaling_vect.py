...
def find_next_stock(df, current_stock_col, current_pop_col, next_pop_col, citytype, t):
    '''
    takes as inputs:
...
#    # Randomly sample loga and beta values
    def sample_coeff(beta_distn):
        beta = np.random.choice(beta_distn, size=1)
        return beta
    ...
    for i in range(100):
        ...
        beta = sample_coeff(beta_uniform)
    ...
    # #------- old method looping through rows------------------
...
def process_stock_at_t(df_for_analysis, current_stock_col, current_pop_col, next_pop_col, t, case, infrastructure):
    '''
    input:
    - current total infrastructure stock per city
    - current population 
    ...
    df_t2_urban = find_next_stock(df_for_analysis, current_stock_col, current_pop_col, next_pop_col, list_of_city_types[0],t)
    df_t2_suburban = find_next_stock(df_for_analysis, current_stock_col, current_pop_col, next_pop_col, list_of_city_types[1],t)
    df_t2_periurban = find_next_stock(df_for_analysis, current_stock_col, current_pop_col, next_pop_col, list_of_city_types[2],t)
    df_t2_rural = find_next_stock(df_for_analysis, current_stock_col, current_pop_col, next_pop_col, list_of_city_types[3],t)
    ...
    print("Columns in df_all_types====================================================")
    print(df_all_types['per_cap_mass_next_time_step'].min(axis=1).head())
    print(df_all_types['per_cap_mass_next_time_step'].mean(axis=1).head())
    print(df_all_types['per_cap_mass_next_time_step'].max(axis=1).head())
    ...
    # if len(df.filter(regex='citytype').columns) != 8:
    if infrastructure == 'RBUV':
        ...
        #     if (row[ua_population_at_t] >= 500000) & (row[HU_density_sqmi_at_t] >= 4000) & (row['ua-to-place allocation factor_max'] > 0.009): # or 0.009??
        #         return 'urban'
